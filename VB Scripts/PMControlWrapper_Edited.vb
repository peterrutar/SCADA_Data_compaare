...
Function PMControlWrapper_LoadRecipe( Byval recipeName )		'Load parameters of the selected recipe
	...
	For i = 0 To count
		...
		Dim tagName: tagName = "Recipe_DB_" & p( i, IDX_NAME )	'Name the WinCC Variables		
		ts.Add tagName
		ts( tagName ).Value = p( i, IDX_VALUE )			'Write recipe value to the WinCC Tag
		
		' Debug
		HMIRuntime.Trace tagName & "->tagName" & vbCrLf

		Dim cbx, ioValue, stLabel, stUnit
		
		'Set stLabel = PMControlWrapper_GetScreenItemByName( "Recipe_DB_" & p( i, IDX_NAME ) )
		...
		Select Case p( i, IDX_TYPE)						'Type selection analog/binary/digital/text
			...
			Case PARAMTYPE_ANALOG 
				Set stUnit = PMControlWrapper_GetScreenItemByName( "Recipe_DB_" & p( i, IDX_NAME ) )
				Set ioValue = PMControlWrapper_GetScreenItemByName( "Recipe_DB_" & p( i, IDX_NAME ) )
			...
			Case PARAMTYPE_BINARY 
				Set cbx = PMControlWrapper_GetScreenItemByName( "Recipe_DB_" & p( i, IDX_NAME ) )
...
'	Next
	...
	PMControlWrapper_LoadRecipe = result
End Function
...
Function PMControlWrapper_SaveRecipe( Byval recipeName )
	...
	Dim si, p
	p = w.GetRecipeValues( recipeName, 0 )
		
	For Each si In HMIRuntime.ActiveScreen.ScreenItems
		
		If Left( si.ObjectName, 10 ) = "Recipe_DB_" Then	'object (IO field) has to be named as it is its recipe value name + prefix "___" (eg. Recipe_DB_)
				...
				For i = 0 To UBound(p, 1)
					...
					names( count ) = p( i, IDX_NAME ) 'Replace Underliner with space for the later filling of the recipe system
					values( count ) = HMIRuntime.Tags( si.ObjectName ).Read
			
					' Debug
					HMIRuntime.Trace names(count).read & vbCrLf
					HMIRuntime.Trace values(count).read & vbCrLf
	...
	result = w.SetRecipeValues( recipeName, tempNames, tempValues, 0 )
