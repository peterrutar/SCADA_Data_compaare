...
Sub Compare()
    ...
    'get all rows
    lastRow = (wsOld.Cells(Rows.Count, 2).End(xlUp).Row) + 1  'column "2" to get all parameters on last tag
    ...
    For i = 1 To lastRow
        ...
        If (IsEmpty(wsOld.Cells(i, 1)) = False) And stringTest <> 0 Then
            ...
            For j = (i + 1) To lastRow
                ...
                stringTest = StrComp(wsOld.Cells(j, 1), "Tag Management", 1)
                
                If ((IsEmpty(wsOld.Cells(j, 1)) = False) And sameVar <> 0 And stringTest <> 0) _
                Or (wsOld.Cells(j, 1).Row = lastRow) Then
            ...
            ReDim oldTag(1 To 10) As String
            ...
            For k = i To j - 1
                ...
                paramTest(8) = StrComp((wsOld.Cells(k, 2).Value), "Start value", 1)
...
'get all rows
lastRow = (wsNew.Cells(Rows.Count, 2).End(xlUp).Row) + 1  'column "2" to get all parameters on last tag
...
For i = 1 To lastRow
    ...
    If compName = 0 Then
        ...
        For j = (i + 1) To lastRow
            ...
            If (IsEmpty(wsNew.Cells(j, 1)) = False) And sameVar <> 0 And stringTest <> 0 _
            Or (wsNew.Cells(j, 1).Row = lastRow) Then
        ...
        ReDim newTag(1 To 10) As String
        Dim paramTest() As Integer
        ReDim paramTest(1 To 8) As Integer
        ...
        For k = i To j - 1
            ...
            paramTest(8) = StrComp((wsNew.Cells(k, 2).Value), "Start value", 1)
...
Function CheckBothParameters(ByRef oldTag() As String, ByRef newTag() As String) As String
    ...
    ReDim nokParameters(1 To 10) As Boolean
    ...
    If (IsEmpty(wsNew.Cells(i, 1)) = False) And stringTest <> 0 Then
        ...
        If isNotPresent Then
            ...
            For k = (i + 1) To lastRow
                ...
                If (IsEmpty(wsNew.Cells(k, 1)) = False) And nextVar <> 0 And stringTest <> 0 _
                Or (wsNew.Cells(j, 1).Row = lastRow) Then
            ...
            For l = i To k - 1
                ...
                paramTest(8) = StrComp((wsNew.Cells(l, 2).Value), "Start value", 1)
...
lastRow = (ActiveSheet.Cells(Rows.Count, 2).End(xlUp).Row) + 1    'column "2" to get all parameters on last tag
